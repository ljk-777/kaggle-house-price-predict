...
score=rmsle_cv(KRR)
print(f"KernelRidge score: {score.mean():.4f}({score.std():.4f})")
score=rmsle_cv(GBoost)
print(f"GradientBoostingRegressor score: {score.mean():.4f}({score.std():.4f})")

lasso.fit(train_data.values, y_train)
enet.fit(train_data.values, y_train)
KRR.fit(train_data.values, y_train)
GBoost.fit(train_data.values, y_train)

def ensemble_predict(X):
    return ((0.25 * lasso.predict(X)) +
            (0.25 * enet.predict(X)) +
            (0.15 * KRR.predict(X)) +
            (0.35 * GBoost.predict(X)))

final_prediction=np.expm1(ensemble_predict(test_data.values))
sub=pd.DataFrame()
sub["ID"]=test_ID
